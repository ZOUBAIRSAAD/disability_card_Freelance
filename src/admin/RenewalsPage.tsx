...
const RenewalsPage: React.FC = () => {
  ...
  const fetchRenewals = async () => {
    try {
      ...
      const [disabilitiesRes, carersRes, customerSupportRes] = await Promise.all([
        fetch('http://api-disability-card.runasp.net/api/admin/renewals/disabilities', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('http://api-disability-card.runasp.net/api/admin/renewals/carers', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('http://api-disability-card.runasp.net/api/admin/renewals/customer-support', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
  ...
  const updateRenewalStatus = async (id: number, type: string, status: string) => {
    try {
      ...
      const response = await fetch(`http://api-disability-card.runasp.net/api/admin/renewals/${urlType}/${id}/status`, {
        method: 'PUT',
        headers: {
