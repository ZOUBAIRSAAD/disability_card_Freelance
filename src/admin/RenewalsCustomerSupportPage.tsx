...
import { Edit, Trash2, Search, Filter, Calendar, Mail, MapPin, Clock, CheckCircle, XCircle, AlertCircle, Headphones, CreditCard, Eye } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { 
  getCustomerSupportRenewals, 
  updateRenewalStatus, 
...
const RenewalsCustomerSupportPage: React.FC = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await getCustomerSupportRenewals();
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await updateRenewalStatus(selectedApplication.id, 'customer_support', {
        status: newStatus,
        adminNotes
      });
  ...
  const handleDelete = async (applicationId: number) => {
    ...
    try {
      await deleteRenewalApplication(applicationId, 'customer_support');
  ...
  const handleViewApplication = async (application: RenewalApplication) => {
    try {
      markAsViewed('renewal-support', application.id);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Customer Support Card Renewals</h1>
        <p className="text-gray-600">Manage customer support card renewal applications</p>
      </div>
              ...
              {applications.map((application) => (
                <tr key={application.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">
                      {application.supportType || 'General Support'}
                    </div>
                    <div className="text-sm text-gray-500 max-w-xs truncate">
                      {application.supportDescription || application.renewalReason}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">Support: {selectedApplication.supportType || 'General'}</p>
                </div>
              </div>
...
export default RenewalsCustomerSupportPage;
