...
import { Edit, Trash2, Search, Filter, Calendar, User, Mail, Clock, CheckCircle, XCircle, AlertCircle, CreditCard, Eye } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { 
  getDisabilityRenewals, 
  updateRenewalStatus, 
...
const RenewalsDisabilitiesPage: React.FC = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await getDisabilityRenewals();
      ...
      console.log('Disability renewals response:', response);
    } catch (error) {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await updateRenewalStatus(selectedApplication.id, 'disability', {
        status: newStatus,
        adminNotes
      });
  ...
  const handleDelete = async (applicationId: number) => {
    ...
    try {
      await deleteRenewalApplication(applicationId, 'disability');
  ...
  const handleViewApplication = async (application: RenewalApplication) => {
    try {
      markAsViewed('renewal-disabilities', application.id);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Disability Card Renewals</h1>
        <p className="text-gray-600">Manage disability card renewal applications</p>
      </div>
                ...
                <tr key={application.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900 max-w-xs">
                      {application.renewalReason}
                    </div>
                    {application.hasChanges && (
                      <div className="text-xs text-blue-600 mt-1">
                        Has changes reported
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">Reason: {selectedApplication.renewalReason}</p>
                </div>
              </div>
...
export default RenewalsDisabilitiesPage;
